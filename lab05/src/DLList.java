...
public class DLList<T> {
    ...
    public void add(int index, T x) {
        //TODO: your code here!
        if (index == 0 || sentinel.next == sentinel) {
            addFirst(x);
            return;
        }
        Node pointer = sentinel;
        for (int i = 0; i < index && pointer.next != sentinel; i++) {
            pointer = pointer.next;
        }
        Node newNode = new Node(x, pointer.next, pointer);
