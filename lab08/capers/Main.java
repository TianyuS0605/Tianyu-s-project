...
public class Main {
    ...
    static final File CAPERS_FOLDER = new File(CWD.getPath() + "/.capers"); // FIXME
    ...
    public static void setupPersistence() {
        // FIXME
        CAPERS_FOLDER.mkdir();
        File dogsFolder = new File(CAPERS_FOLDER.getPath() + "/dogs");
        dogsFolder.mkdir();
        try {
            File story = new File(CAPERS_FOLDER.getPath() + "/story");
            story.createNewFile();
        } catch (IOException e) {
            System.out.println("Failed to create story file");
        }
    }
    ...
    public static void writeStory(String[] args) {
        ...
        // FIXME
        String contentToWrite = Utils.readContentsAsString(new File(CAPERS_FOLDER.getPath() + "/story")) + args[1] + "\n";
        Utils.writeContents(new File(CAPERS_FOLDER.getPath() + "/story"), contentToWrite);
        System.out.println(Utils.readContentsAsString(new File(".capers/story")));
    }
    ...
    public static void makeDog(String[] args) {
        ...
        // FIXME
        try {
            Dog newDog = new Dog(args[1], args[2], parseInt(args[3]));
            Utils.writeObject(new File(CAPERS_FOLDER.getPath() + "/dogs/" + args[1]), newDog);
    ...
    public static void celebrateBirthday(String[] args) {
        ...
        // FIXME
        try {
            Dog currentDog = Dog.fromFile(args[1]);
            currentDog.haveBirthday();
            currentDog.saveDog();
        } catch (Exception e) {
            exitWithError("Fail to celebrate birthday for " + args[1]);
        }
    }
